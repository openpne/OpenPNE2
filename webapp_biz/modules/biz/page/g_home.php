...
class biz_page_g_home extends OpenPNE_Action
{
    function execute($requests)
    {
        ...
        $this->set('calendar_head', biz_getScheduleWeek($target_id, $u, $requests['w'], 'g', ture, false, false));

        foreach ($member_list_full as $key => $value) {
            $calendar_value .= biz_getScheduleWeek($value['c_member_id'], $u, $requests['w'], 'g', false, true, false, $value);
        }

        $this->set('calendar_value', $calendar_value);
        $this->set('calendar_foot', biz_getScheduleWeek($target_id, $u, $requests['w'], 'g', false, false, true));
