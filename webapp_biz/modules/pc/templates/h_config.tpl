...
<input type="radio" name="is_receive_ktai_mail" value="1"({if $c_member.is_receive_ktai_mail}) checked="checked"({/if}) class="no_bg">転送する
<input type="radio" name="is_receive_ktai_mail" value="0"({if !$c_member.is_receive_ktai_mail}) checked="checked"({/if}) class="no_bg">転送しない<br>
<span>※携帯メールアドレスを登録していただいている場合、携帯にもメッセージが転送されます。</span>
