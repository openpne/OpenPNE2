...
({assign var="page_name" value="トピックコメント削除"})
...
このトピックコメントを削除します。削除後に復元させることはできないので注意してください。

<form action="./" method="post">
<input type="hidden" name="sessid" value="({$PHPSESSID})" />
<input type="hidden" name="m" value="({$module_name})" />
<input type="hidden" name="a" value="do_({$hash_tbl->hash('delete_kakikomi_c_commu_topic_comment')})" />
...
({/if})
({$topic_comment.body|t_truncate:"120"|nl2br})
</td>
</tr>
({****})
</tbody>
