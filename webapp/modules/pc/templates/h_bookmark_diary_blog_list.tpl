...
<a href="({t_url m=pc a=page_fh_diary})&amp;target_c_diary_id=({$item.c_diary_id})&amp;comment_count=({$item.count_comments})">({$item.subject}) (({$item.count_comments}))</a>
...
<a href="({$item.link})" target="_blank">({$item.subject})</a> (({$item.nickname}))
