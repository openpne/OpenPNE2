...
({if $is_h_prof == 1})
<font color="#ff0000">※他のﾒﾝﾊﾞｰから見たあなたのﾍﾟｰｼﾞはこのようになります。<br>ﾌﾟﾛﾌｨｰﾙを変更する場合は <a href="({t_url m=ktai a=page_h_config_prof})&amp;({$tail})">「ﾌﾟﾛﾌｨｰﾙ変更」 </a>よりおこなえます。</font><br>
({/if})
...
<img src="({t_img_url filename=$rank.image_filename w=$smarty.const.OPENPNE_IMG_KTAI_MAX_WIDTH h=$smarty.const.OPENPNE_IMG_KTAI_MAX_HEIGHT})" alt="({$rank.name})" width="120"><br>
({/if})
...
({strip})
({capture name="birth"})
({if $target_c_member.age !== NULL})<font color="#999966">年齢：</font>({$target_c_member.age})歳<br>
({if $is_h_prof && $target_c_member.public_flag_birth_year == 'friend'})<font color="#ff0000">※({$WORD_MY_FRIEND_HALF})まで公開</font><br>({/if})
({/if})
