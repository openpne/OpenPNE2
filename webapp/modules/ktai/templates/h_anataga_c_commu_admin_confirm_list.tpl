...
<font color="#({$ktai_color_config.color_24})">ｺﾐｭﾆﾃｨ管理者交代要請中</font><br>
</td></tr></table>
({if !$anataga_c_commu_admin_confirm_list})
現在あなたが管理しているｺﾐｭﾆﾃｨの管理者交代を要請しているﾒﾝﾊﾞｰはいません。
({else})
<center>
...
({if $is_prev || $is_next})
<center>
({if $is_prev})<a href="({t_url m=ktai a=page_h_anataga_c_commu_admin_confirm_list})&amp;target_c_member_id=({$target_c_member.c_member_id})&amp;page=({$page-1})&amp;({$tail})" accesskey="4">[i:128]前を表示</a>({/if})
({if $is_prev && $is_next})&nbsp;({/if})
({if $is_next})<a href="({t_url m=ktai a=page_h_anataga_c_commu_admin_confirm_list})&amp;target_c_member_id=({$target_c_member.c_member_id})&amp;page=({$page+1})&amp;({$tail})" accesskey="6">[i:130]次を表示</a>({/if})
<br>
</center>
({/if})
