...
<font color="#({$ktai_color_config.font_05})"><a name="top">承認待ち</a></font><br>
</td></tr>
<tr><td bgcolor="#({$ktai_color_config.bg_03})" align="center">
<font color="#({$ktai_color_config.color_24})">({$WORD_COMMUNITY_HALF})副管理者承認待ち</font><br>
</td></tr></table>
({if !$anatani_c_commu_sub_admin_confirm_list})
現在あなたに({$WORD_COMMUNITY_HALF})副管理者を要請しているﾒﾝﾊﾞｰはいません。
({else})
<center>
...
({if $is_prev || $is_next})
<center>
({if $is_prev})<a href="({t_url m=ktai a=page_h_anatani_c_commu_sub_admin_confirm_list})&amp;target_c_member_id=({$target_c_member.c_member_id})&amp;page=({$page-1})&amp;({$tail})" accesskey="4">[i:128]前を表示</a>({/if})
({if $is_prev && $is_next})&nbsp;({/if})
({if $is_next})<a href="({t_url m=ktai a=page_h_anatani_c_commu_sub_admin_confirm_list})&amp;target_c_member_id=({$target_c_member.c_member_id})&amp;page=({$page+1})&amp;({$tail})" accesskey="6">[i:130]次を表示</a>({/if})
<br>
</center>
({/if})
