...
function smarty_function_t_img_url_skin_biz($params, &$smarty)
{
    $p = _smarty_function_t_img_url_skin_biz($params);
    ...
    if (!$filename = db_get_c_skin_filename4skinname($p['filename'])) {
        ...
        $url .= sprintf('skin/biz/%s.%s', $p['filename'], $ext);
    } else {
        $p['filename'] = $filename;
 ...
 * @return array
 */
function _smarty_function_t_img_url_skin_biz($params)
{
    $result = array();
